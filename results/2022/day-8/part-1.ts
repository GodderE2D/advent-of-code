...
// https://github.com/tymscar/Advent-Of-Code/blob/master/2022/typescript/day08/part1.ts
...
const isVisible = (x: number, y: number, forest: number[][]): number => {
  ...
  const tallTrees = [left, up, right, down].map((direction) => Math.max(...direction));
...
const treeVisibilities = trees.map((line, index) => line.map((_, i) => isVisible(i, index, trees)));
